...
package uk.ac.man.cs.comp38120.util.map;
...
public class HMapID implements MapID, Cloneable, Serializable
{
    ...
    /**
     * The number of times this HMapID has been structurally modified Structural
     * modifications are those that change the number of mappings in the HMapID
     * or otherwise modify its internal structure (e.g., rehash). This field is
     * used to make iterators on Collection-views of the HMapID fail-fast. (See
     * ConcurrentModificationException).
     */
    transient volatile int modCount;

    /**
     * Constructs an empty <tt>HMapID</tt> with the specified initial capacity
     * and load factor.
     * 
    ...
    /**
     * Constructs an empty <tt>HMapID</tt> with the default initial capacity
    ...
    /**
     * Constructs a new <tt>HMapID</tt> with the same mappings as the specified
     * <tt>MapID</tt>. The <tt>HMapID</tt> is created with default load factor
     * (0.75) and an initial capacity sufficient to hold the mappings in the
     * specified <tt>MapID</tt>.
     * 
     * @param m
     ...
     * constructors and pseudo-constructors (clone, readObject) after HMapID has
     ...
     * against poor quality hash functions. This is critical because HMapID uses
    ...
    /**
     * Returns the entry associated with the specified key in the HMapID.
     * Returns null if the HMapID contains no mapping for the key.
     */
    ...
    {
        for (Iterator<? extends MapID.Entry> i = m.entrySet().iterator(); i
                .hasNext();)
        {
     ...
     * HMapID. Returns null if the HMapID contains no mapping for this key.
     */
    ...
    /**
     * Returns a shallow copy of this <tt>HMapID</tt> instance: the keys and
     ...
     * 
     * Subclass overrides this to alter the behavior of HMapID(Map), clone, and
     * readObject.
     */
    ...
    /**
     * Save the state of the <tt>HMapID</tt> instance to a stream (i.e.,
     * serialize it).
     * 
     * @serialData The <i>capacity</i> of the HMapID (the length of the bucket
    ...
    /**
     * Reconstitute the <tt>HMapID</tt> instance from a stream (i.e.,
     * deserialize it).
     */
