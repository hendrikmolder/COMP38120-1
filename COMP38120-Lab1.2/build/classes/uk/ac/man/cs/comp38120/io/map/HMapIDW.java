...
package uk.ac.man.cs.comp38120.io.map;
...
import uk.ac.man.cs.comp38120.util.map.HMapID;
import uk.ac.man.cs.comp38120.util.map.MapID;
 ...
 * 
 * <p>
 * One notable feature of this class is the ability to support <i>lazy
 * decoding</i>, controlled by the {@link #setLazyDecodeFlag(boolean)} method.
 * In lazy decoding mode, when an object of this type is deserialized, key-value
 * pairs are not inserted into the map, but rather held in arrays. The reduces
 * memory used in cases where random access to values is not required. In lazy
 * decoding mode, the raw keys and values may be fetched by the
 * {@link #getKeys()} and {@link #getValues()} methods, respectively. The map
 * can be subsequently populated with the {@link #decode()} method.
 * </p>
 * 
 * @author Jimmy Lin
 */
public class HMapIDW extends HMapID implements Writable
{
    ...
    /**
     * Creates a <code>HMapIDW</code> object.
     */
    public HMapIDW()
    {
        super();
    ...
    {
        if (getLazyDecodeFlag() == false) return true;
    ...
    /**
     * Creates a <code>HMapIDW</code> object from a <code>DataInput</code>.
     * 
     * @param in
     *            source for reading the serialized representation
     * @return a newly-created <code>HMapIDW</code> object
     * @throws IOException
     */
    public static HMapIDW create(DataInput in) throws IOException
    {
    ...
    /**
     * Creates a <code>HMapIDW</code> object from a byte array.
     * 
     * @param bytes
     ...
     * @throws IOException
     */
    public static HMapIDW create(byte[] bytes) throws IOException
    {
